...
async def policy_lookup_agent(state: ARIAState) -> dict:
    ...
    elif any(r.appetite_status == "decline" for r in all_rules):
        appetite = "DECLINE"
    elif any(r.appetite_status == "refer"   for r in all_rules):
        appetite = "REFER"
    else:
    ...
    # ── HITL trigger ──────────────────────────────────────────────
    requires_senior = (
        sum_insured > settings.hitl_sum_insured_threshold or
        appetite == "REFER" or
        confidence < settings.hitl_confidence_threshold or
        len(gaps) > 2
    )
